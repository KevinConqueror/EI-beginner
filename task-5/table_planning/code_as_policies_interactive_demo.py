...
# Initialize OpenAI client with new API structure
...
class LMP:
    ...
    def build_prompt(self, query, context=''):
        ...
        prompt += f'\n{use_query}'
        # prompt = f'<|fim_prefix|>{prompt}<|fim_suffix|>'
    ...
    def __call__(self, query, context='', **kwargs):
        ...
        while True:
            try:
                ...
                code_str = response.choices[0].text.strip()
                break
...
class LMPFGen:
    ...
    def create_f_from_sig(self, f_name, f_sig, other_vars=None, fix_bugs=False, return_src=False):
        ...
        # prompt = f'<|fim_prefix|>{self._base_prompt}\n{use_query}<|fim_suffix|>'
        ...
        while True:
            try:
                response = client.completions.create(
                    ...
                    stop=self._stop_tokens,
                    temperature=self._cfg['temperature'],
                    model=self._cfg['engine'],  # Using model instead of engine
                    max_tokens=self._cfg['max_tokens']
                )
                f_src = response.choices[0].text.strip()
                break
            except (openai.RateLimitError, openai.APIConnectionError, openai.APIError) as e:
                print(f'OpenAI API got err {e}')
                print('Retrying after 10s.')
        ...
        if fix_bugs:
            #fix_prompt = f'<fim_prefix|># {f_src}\nFix the bug if there is one. Improve readability. Keep same inputs and outputs. Only small changes. No comments.<|fim_suffix|>'
            ...
            f_src = response.choices[0].text.strip()
            print("Note: OpenAI Edit API has been deprecated, skipping bug fixing.")
...
2. Input a command in the `user_input` field below and run the cell. This will run Code as Policies by querying the code-generating LLM to complete the task.
...
# Initialize Env
num_blocks = 3  # Number of blocks (0-4)
num_bowls = 3   # Number of bowls (0-4)
...
# Display env using OpenCV instead of cv2_imshow
camera_img = env.get_camera_image()
cv2.imshow('Environment', cv2.cvtColor(camera_img, cv2.COLOR_BGR2RGB))
...
# Interactive Demo
user_input = 'Put the pink block to the left of the rightmost bowl'  # Command to execute
...
# Show the final environment state
final_img = env.get_camera_image()
cv2.imshow('Final State', cv2.cvtColor(final_img, cv2.COLOR_BGR2RGB))
cv2.waitKey(0)
cv2.destroyAllWindows()

# Save and play video if cache exists
if env.cache_video:
    from moviepy.video.io.ImageSequenceClip import ImageSequenceClip  # Updated import path
    rendered_clip = ImageSequenceClip(env.cache_video, fps=35 if high_frame_rate else 25)
    # Save the video file instead of displaying inline
    rendered_clip.write_videofile("simulation_output.mp4", codec='libx264')
    print("Video saved as simulation_output.mp4")
